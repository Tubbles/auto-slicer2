...
class TestPackModels:
    def test_single_model(self, tmp_path):
        ...
        beds, rejected = pack_models([stl], 235, 235, {"adhesion_type": "skirt", "skirt_distance": "3"})
        assert len(beds) == 1
        assert len(beds[0]) == 1
        assert rejected == []
    ...
    def test_two_models_fit_one_bed(self, tmp_path):
        ...
        beds, rejected = pack_models([a, b], 235, 235, {"adhesion_type": "skirt", "skirt_distance": "3"})
        assert len(beds) == 1
        assert len(beds[0]) == 2
        assert rejected == []
    ...
    def test_models_overflow_to_second_bed(self, tmp_path):
        ...
        beds, rejected = pack_models(models, 235, 235, {"adhesion_type": "skirt", "skirt_distance": "3"})
        assert len(beds) >= 2
    ...
    def test_oversized_model_rejected(self, tmp_path):
        ...
        beds, rejected = pack_models([small, huge], 235, 235, {"adhesion_type": "none"})
        assert len(beds) == 1
        assert len(beds[0]) == 1  # only the small one packed
        assert len(rejected) == 1
        assert rejected[0][0] == huge
        assert rejected[0][1] == "too large for bed"
