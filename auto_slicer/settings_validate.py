...
class SettingsValidator:
    def validate(self, defn: SettingDefinition, raw_value: str) -> ValidationResult:
        dispatch = {
        ...
        handler = dispatch.get(defn.setting_type, self._validate_str)
        return handler(defn, raw_value)

    def _validate_float(self, defn: SettingDefinition, raw: str) -> ValidationResult:
        try:
            val = float(raw)
        except ValueError:
            return ValidationResult(ok=False, coerced_value=raw,
                                    error=f"Expected a number, got '{raw}'")
        return self._check_bounds(defn, val, raw)

    def _validate_int(self, defn: SettingDefinition, raw: str) -> ValidationResult:
        try:
            val = int(raw)
        except ValueError:
            ...
            try:
                ...
                return ValidationResult(ok=False, coerced_value=raw,
                                        error=f"Expected an integer, got '{raw}'")
        return self._check_bounds(defn, val, str(val))

    def _check_bounds(self, defn: SettingDefinition, val: float, coerced: str) -> ValidationResult:
    ...
    def _validate_bool(self, defn: SettingDefinition, raw: str) -> ValidationResult:
    ...
    def _validate_enum(self, defn: SettingDefinition, raw: str) -> ValidationResult:
    ...
    def _validate_str(self, defn: SettingDefinition, raw: str) -> ValidationResult:
        return ValidationResult(ok=True, coerced_value=raw)
