...
- `pynest2d` library (system package — NFP-based 2D nesting for batch model layout)
...
auto_slicer/
  ...
  packing.py               # Bin-pack models onto print beds via pynest2d
...
**Batch packing** (`packing.py`): When `batch_models` is enabled, `pack_models()` computes each STL's 2D convex hull (XY projection), adds adhesion margin (skirt_distance, brim_width, or raft_margin depending on adhesion_type) plus an inter-model gap, and uses `pynest2d` (libnest2d NFP algorithm) to nest hulls into bed-sized bins. Returns a list of beds, each containing `[(path, offset_x, offset_y), ...]` with offsets relative to bed center. Models that cannot be placed are silently omitted. `slice_batch()` then invokes CuraEngine with multiple `-l` flags, each followed by per-mesh `-s mesh_position_x=... -s mesh_position_y=...` settings.
...
7. If `batch_models` is enabled and multiple models are present, `pack_models()` nests their convex hulls onto beds using `pynest2d`, then `slice_batch()` invokes CuraEngine once per bed with multiple `-l` flags and per-mesh `mesh_position_x/y` offsets
