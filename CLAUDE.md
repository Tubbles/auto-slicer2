...
**Batch packing** (`packing.py`): When `batch_models` is enabled, `pack_models()` computes each STL's 2D convex hull (XY projection), adds adhesion margin (skirt_distance, brim_width, or raft_margin depending on adhesion_type) plus an inter-model gap, and uses `pynest2d` (libnest2d NFP algorithm) to nest hulls into bed-sized bins. Returns `(beds, rejected)` where beds is a list of `[(path, offset_x, offset_y), ...]` with offsets relative to bed center, and rejected is a list of `(path, reason)`. `slice_batch()` then invokes CuraEngine with multiple `-l` flags, each followed by per-mesh `-s mesh_position_x=... -s mesh_position_y=...` settings.
